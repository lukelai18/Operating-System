...
{
    vunlock(file);
    ssize_t tmp=file->vn_dev.chardev->cd_ops->read(file->vn_dev.chardev, pos, buf, count);
    vlock(file);
...
{
    vunlock(file);
    ssize_t tmp=file->vn_dev.chardev->cd_ops->write(file->vn_dev.chardev, pos, buf, count);
    vlock(file);
...
{
    /* VM {{{ */
    return file->vn_dev.chardev->cd_ops->mmap(file, ret);
    /* VM }}} */
    return 0;
}
...
{
    /* VM {{{ */
    return file->vn_dev.chardev->cd_ops->fill_pframe(file, pf);
    /* VM }}} */
    return 0;
}
...
{
    /* VM {{{ */
    return file->vn_dev.chardev->cd_ops->flush_pframe(file, pf);
    /* VM }}} */
    return 0;
}
